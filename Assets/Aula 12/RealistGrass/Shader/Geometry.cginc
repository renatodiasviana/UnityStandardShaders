...
v2g CalculateVertex(v2g input, float colorIntensity, float2 uv, float4 vertexOffSet)
{
	v2g v = input;
...
void generateGrass(v2g vData, float4 wind, float noise, float3 offset, inout TriangleStream<g2f> triStream)
{
	...
	{
		v1 = CalculateVertex(vData, colorIntensity, float2(0.0, uvY), float4(offset, 0.0) + segmentHight + windAnimation);
		v2 = CalculateVertex(vData, colorIntensity, float2(1.0, uvY), float4(offset * -1.0, 0.0) + segmentHight + windAnimation);

		// Updating data for the upper vertex
		float noiseHight = lerp(0.5, 1.0, CalculateRandomValue(noise)) * _GrassHight;
		colorIntensity += 1.0 / _GrassSegmentCount;
		uvY += 1.0 / _GrassSegmentCount;

		windAnimation += wind * (1.0 / _GrassSegmentCount);

		v3 = CalculateVertex(vData, colorIntensity, float2(0.0, uvY),
							float4(offset, 0.0) + float4(vData.normal * noiseHight, 0.0) + segmentHight + windAnimation);

		v4 = CalculateVertex(vData, colorIntensity, float2(1.0, uvY),
							float4(offset * -1.0, 0.0) + float4(vData.normal * noiseHight, 0.0) + segmentHight + windAnimation);
...
void geom(triangle v2g IN[3], inout TriangleStream<g2f> triStream)
{
	...
	for (int i = 0; i < 5; i++)
	{
		v2g baryCentricAlt = baryCentric;

		float randomValue = CalculateRandomValue(0.0, 10.0);
		float randomAngle = CalculateRandomValue(baryCentricAlt.pos.zzx * randomValue) * 360.0;
		...
		float4 noise = tex2Dlod(_HightNoiseTex, float4(baryCentricAlt.uv, 0, 0));

		float2 windUV = baryCentricAlt.uv + float2(0.15, 0.15) * (sin(_Time.y * _WindSpeedController * 2.0)) * _WindFrequencyController * 0.5;
		...
		float4 collisionMask = tex2Dlod(_CollisionDirTex, float4(baryCentricAlt.uv, 0, 0));

		float4 distortion = wind * 0.1;
		if (length(collisionMask.rgb) != 0.0)
		{
			collisionMask = float4(UnpackNormal(collisionMask), 0.0);
			distortion = collisionMask * 0.15;
		}

		generateGrass(baryCentricAlt, distortion, noise.x, offset, triStream);
	}
}
