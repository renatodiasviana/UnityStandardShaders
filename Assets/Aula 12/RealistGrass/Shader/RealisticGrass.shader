Shader "Unlit/RealisticGrass"
{
	Properties
	{
		...
		_WindSpeedController("Wind Speed Controller", Range(0, 1.0)) = 0.3
		...
		_AlphaCutout("Alpha Cutout Value", Range(0.1, 1.0)) = 1.0
	}
	SubShader
	{
		Pass
		{
			...
			fixed4 frag(g2f i) : SV_Target
			{
				...
				float3 specularColor = col.rgb * _LightColor0.rgb * OneMinusNdotV * i.color;
		...
		Pass
		{
			...
			fixed4 frag(g2f i) : SV_Target
			{
				...
				// Calculate Diffuse light
				half NdotL = round(clamp(dot(i.normal, _WorldSpaceLightPos0.xyz), 0.0, 1.0) * _ShadingTones) / _ShadingTones;
				float3 diffuseColor = col.rgb * _LightColor0.rgb * NdotL * i.color;
